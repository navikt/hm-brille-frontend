...
import {Button, Panel, Heading, Alert, Loader} from "@navikt/ds-react"
...
export function OversiktDetaljer() {
    ...
    const {
        data,
        error,
    } = useGet<OversiktDetaljerResponse>('/oversikt/' + vedtakId)
                ...
                {!error && data && (
                    <>
                        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center' }}>
                            <h1 style={{ display: 'inline-block', marginRight: '1rem', margin: '0.1em 0' }}>Krav for {data.barnsNavn}</h1>
                            <Button
                                variant="secondary"
                                size="medium"
                                className="dontPrintMe"
                        ...
                        <Panel border={true} style={{ marginTop: '1rem' }}>
                            ...
                            <div style={{ marginTop: '2rem' }}>
                                ...
                                <Data>
                                    ...
                                    <DatumHelper label="Venstre sylinder">{data.venstreSylinder.toFixed(2).replace('.', ',')}</DatumHelper>
                                </Data>
                            </div>
                            <div style={{ marginTop: '2rem' }}>
                                <Heading level="1" size="medium">
                            ...
                            <div style={{ marginTop: '2rem', marginBottom: '1rem' }}>
                                ...
                                <Line />
                                <ul>
                                    <li>Barnet kan få støtte fra sats {data.satsNr}: {data.satsBeskrivelse}</li>
                                    <li>Barnet har krav på brillestøtte på kr. {data.belop.toFixed(2).replace('.', ',')}</li>
                                </ul>
                                <Heading level="1" size="small">Krav om direkte oppgjør fra NAV på kr. {data.belop.toFixed(2).replace('.', ',')}</Heading>
                            </div>
                        </Panel>
                    </>
                )}
...
const DatumHelper = (props: DatumHelperProps) => {
    return (
        <Datum label={props.label}>
            {props.children && (
                <span style={{ marginLeft: '1rem' }}>{props.children}</span>
            )}
        </Datum>
    )
}
